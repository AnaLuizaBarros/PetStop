...
CREATE TABLE [Animal] (
    [Animal_ID] int NOT NULL IDENTITY,
    [Nome] VARCHAR(50) NOT NULL,
    [RacaId_Raca] int NULL,
    CONSTRAINT [PK_Animal] PRIMARY KEY ([Animal_ID]),
    CONSTRAINT [FK_Animal_Especie_Animal_ID] FOREIGN KEY ([Animal_ID]) REFERENCES [Especie] ([Id_Especie]) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX [idx_Animal_ID] ON [Animal] ([Animal_ID]);
GO
...
ALTER TABLE [Animal] DROP CONSTRAINT [FK_Animal_especie_Animal_ID];
GO
...
EXEC sp_rename N'[Animal].[Animal_ID]', N'Animal_ID', N'COLUMN';
GO
...
INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210919144236_ThirdMigration', N'5.0.9');
GO
